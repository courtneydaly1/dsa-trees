...
class BinaryTree {
  ...
  minDepth() {
    ...
    function minFinder(node) {
      if(node.left === null && node.right == null) return 1;
      if (node.left === null) return minFinder(node.right) + 1;
      if (node.right === null) return minFinder(node.left) + 1;
      return(
        Math.min(minFinder(node.left), minFinder(node.right)) +1
      );
    }
    return minFinder(this.root);
  }
  ...
  maxDepth() {
    ...
    function maxFinder(node) {
      if (node.left === null && node.right === null) return 1;
      if (node.left === null) return maxFinder(node.right) + 1;
      if (node.right === null) return maxFinder(node.left) + 1;
      return (
        Math.max(maxFinder(node.left), maxFinder(node.right)) + 1
      );
    }

    return maxFinder(this.root);
  }
  ...
  maxSum() {
    ...
    function maxFinder(node) {
      ...
      const leftSum = maxFinder(node.left);
      const rightSum = maxFinder(node.right);
